          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(attr,index) in spuSaleAttrList" :key="attr.id">
                ...
                v-for="(value,index) in attr.spuSaleAttrValueList" :key="value.id"
                @click="switchValue(attr.spuSaleAttrValueList,value)"
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <button class="mins" @click="skuNum--" :disabled='skuNum<=1'>-</button> -->
...
<script>
  ...
  export default {
    ...
    mounted(){
      this.$store.dispatch('getSkuDetail',this.skuId)
    },

    // 配置计算属性
    computed: {
      // 通过mapGetters函数得到需要的数据
      ...mapGetters(['categoryView','spuSaleAttrList','skuInfo'])
    },

    methods: {
      // 切换选择属性的回调函数
      switchValue(attr,value){
        attr.forEach(item => {
          item.isChecked = '0'
        })
        value.isChecked = '1'
      ...
      async addToCart(){
        ...
        // 向后台发送添加商品到购物车的请求
        let result = await this.$api.postSqlAddCart({skuId,skuNum})
        if (result.code === 200) { // 添加成功
          this.$router.replace({path:'/addcartsuccess',query:{skuId,skuNum}})  // 跳转到添加购物车成功路由
          // 路由组件间通过sessionStorage携带商品信息数据
